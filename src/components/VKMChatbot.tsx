...
const GREETING =
  "Hello! I'm the VKM AI Advisor. I can answer questions about GST, Income Tax, TDS, Labour Law, Company Registration and more. What would you like to know?";
...
export default function VKMChatbot() {
  ...
  function handleSend(text: string) {
    ...
    const botMsg: Message = answer
      ...
      : {
          from: "bot",
          text: "I don't have a specific answer for that. Would you like to connect with CMA Vicky Munje directly?",
          escalate: true,
        };
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 left-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-teal text-white shadow-xl transition-transform hover:scale-110"
      >
        {open ? (
      ...
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-24 left-4 z-50 flex w-80 flex-col rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 sm:w-96">
          {/* Header */}
          <div className="flex items-center gap-3 rounded-t-2xl bg-teal px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/70">V K Munje & Company · Always online</p>
            </div>
            <button
              ...
              <div key={i} className={`flex flex-col ${msg.from === "user" ? "items-end" : "items-start"}`}>
                ...
                {msg.escalate && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    <a
                      href={`https://wa.me/${wa}?text=${encodeURIComponent("Hello, I need expert advice.")}`}
                      target="_blank"
                    ...
                    <a
                      ...
                      className="flex items-center gap-1.5 rounded-full bg-teal px-3 py-1.5 text-xs font-semibold text-white"
                    >
                      <FaPhone className="h-3 w-3" /> Call Now
                    </a>
                    <a
                      href="/book"
                      className="flex items-center gap-1.5 rounded-full bg-gold px-3 py-1.5 text-xs font-semibold text-white"
                    >
                      <FaCalendarCheck className="h-3.5 w-3.5" /> Book Consultation
                    </a>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {SUGGESTED.map((q) => (
                  <button
                    ...
                    className="rounded-full border border-teal/30 bg-cream-deep px-2.5 py-1 text-xs text-teal transition-colors hover:bg-teal hover:text-white"
                  >
                    {q}
                  </button>
                ))}
          ...
          <div className="flex items-center gap-2 border-t border-black/5 p-3">
            ...
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal text-white transition-transform hover:scale-105"
              aria-label="Send"
            >
